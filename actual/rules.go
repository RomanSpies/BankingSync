...
// Apply runs the rule's actions and returns how many were actually applied.
func (r *Rule) Apply(d *DB, t *Transaction) int {
	applied := 0
...
func applyAction(d *DB, t *Transaction, a Action) error {
	...
	case "cleared":
		...
		t.Cleared = v != 0
	default:
		return fmt.Errorf("unsupported rule action field %q", a.Field)
	}
	return nil
}
