...
func TestMerge_keepsOwnClientID(t *testing.T) {
	local := NewHULCClient()
	ownID := local.ClientID

	remote, err := HULCFromTimestamp("2026-07-01T10:00:00.000Z-0005-FOREIGNCLIENT01")
	if err != nil {
		t.Fatalf("parse remote: %v", err)
	}
	local.Merge(remote)

	if local.ClientID != ownID {
		t.Errorf("client ID: got %q, want our own %q — adopting a peer ID breaks HULC uniqueness", local.ClientID, ownID)
	}
}

func TestMerge_counterNeverGoesBackwards(t *testing.T) {
	local := NewHULCClient()
	local.InitialCount = 0x0500

	remote, err := HULCFromTimestamp("2026-07-01T10:00:00.000Z-0003-FOREIGNCLIENT01")
	if err != nil {
		t.Fatalf("parse remote: %v", err)
	}
	local.Merge(remote)

	if local.InitialCount < 0x0500 {
		t.Errorf("counter went backwards: got %d, want >= %d", local.InitialCount, 0x0500)
	}
}

func TestMerge_advancesPastRemoteCounter(t *testing.T) {
	local := NewHULCClient()
	local.InitialCount = 2

	remote, err := HULCFromTimestamp("2026-07-01T10:00:00.000Z-000A-FOREIGNCLIENT01")
	if err != nil {
		t.Fatalf("parse remote: %v", err)
	}
	local.Merge(remote)

	if local.InitialCount != 0x0A+1 {
		t.Errorf("counter: got %d, want %d (must exceed the remote counter)", local.InitialCount, 0x0A+1)
	}
}

func TestMerge_adoptsLaterRemoteTimestamp(t *testing.T) {
	local := NewHULCClient()
	remote, err := HULCFromTimestamp("2026-07-01T10:00:00.000Z-0001-FOREIGNCLIENT01")
	if err != nil {
		t.Fatalf("parse remote: %v", err)
	}
	local.Merge(remote)

	if !local.TS.Equal(remote.TS) {
		t.Errorf("TS: got %v, want %v", local.TS, remote.TS)
	}
}

func TestMerge_nilIsNoop(t *testing.T) {
	local := NewHULCClient()
	local.InitialCount = 7
	local.Merge(nil)
	if local.InitialCount != 7 {
		t.Errorf("counter changed on nil merge: got %d", local.InitialCount)
	}
}
