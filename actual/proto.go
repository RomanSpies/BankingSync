...
// EncodeSyncResponseForTest builds a wire-format sync response, mirroring what
// the Actual Budget server returns. It exists so client tests can drive the
// real decode path instead of a hand-rolled approximation.
func EncodeSyncResponseForTest(envelopes []MessageEnvelope, merkle string) ([]byte, error) {
	var b []byte
	for i := range envelopes {
		b = appendBytes(b, 1, envelopes[i].encode())
	}
	b = appendStr(b, 2, merkle)
	return b, nil
}
