...
func TestVersionPattern(t *testing.T) {
	tests := []struct {
		...
		// A major bump must not switch the update check off. Pinning the pattern
		// to one major makes every new release invisible: the tags stop matching,
		// the newest release of the old major keeps looking current, and nothing
		// reports an error.
		{"2.26.34.7", true},
		{"10.26.34.7", true},
		{"dev-2.26.34.7", false},
...
// The scheme is major.year.week.pipeline, so a bump has to beat every release of
// the previous major regardless of how high its year or week ran.
func TestIsNewer_majorBumpBeatsThePreviousMajor(t *testing.T) {
	if !isNewer("2.26.1.1", "1.99.52.9999") {
		t.Error("a 2.x release did not register as newer than the highest 1.x")
	}
	if isNewer("1.99.52.9999", "2.26.1.1") {
		t.Error("a 1.x release registered as newer than a 2.x one")
	}
}
