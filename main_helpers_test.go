...
func TestSanitiseHeader_stripsCRLF(t *testing.T) {
	got := sanitiseHeader("Subject\r\nBcc: attacker@evil.example")
	if strings.ContainsAny(got, "\r\n") {
		t.Errorf("got %q — CR/LF must never survive into an SMTP header", got)
	}
}

func TestSanitiseHeader_leavesPlainTextAlone(t *testing.T) {
	const in = "BankingSync: ING session expires in 5 days"
	if got := sanitiseHeader(in); got != in {
