...
func TestEarliestPendingDate_empty(t *testing.T) {
	s := &State{PendingMap: map[int64]map[string]string{}}
	_, ok := s.EarliestPendingDate(1)
	if ok {
...
func TestEarliestPendingDate_single(t *testing.T) {
	s := &State{PendingMap: map[int64]map[string]string{1: {
		"REF-1": "txn-1|2024-06-15",
	}}}
	d, ok := s.EarliestPendingDate(1)
	if !ok {
...
func TestPendingKey_refDisambiguatesSameAmountAndDate(t *testing.T) {
	s := &State{PendingMap: make(map[int64]map[string]string)}
	st := newTestStore(t)

	if err := s.SetPending(1, "REF-A", "txn-1", "2024-06-15", st); err != nil {
		t.Fatal(err)
	}
	if err := s.SetPending(1, "REF-B", "txn-2", "2024-06-15", st); err != nil {
		t.Fatal(err)
	}

	if len(s.Pending(1)) != 2 {
		t.Fatalf("expected 2 pending entries, got %d", len(s.Pending(1)))
	}

	idA, _ := splitPendingVal(s.Pending(1)["REF-A"])
	idB, _ := splitPendingVal(s.Pending(1)["REF-B"])
	if idA != "txn-1" {
	...
	if err := s.DeletePending(1, "REF-A", st); err != nil {
		t.Fatal(err)
	}
	if _, exists := s.Pending(1)["REF-A"]; exists {
		t.Error("REF-A should be deleted")
	}
	idB2, _ := splitPendingVal(s.Pending(1)["REF-B"])
	if idB2 != "txn-2" {
...
func TestPendingKey_emptyRefCollides(t *testing.T) {
	...
	if err := s.SetPending(1, key, "txn-1", "2024-06-15", st); err != nil {
		t.Fatal(err)
	}
	if err := s.SetPending(1, key, "txn-2", "2024-06-15", st); err != nil {
		t.Fatal(err)
	}

	if len(s.Pending(1)) != 1 {
		t.Fatalf("expected 1 pending entry (collision), got %d", len(s.Pending(1)))
	}
	id, _ := splitPendingVal(s.Pending(1)[key])
	if id != "txn-2" {
...
func TestPruneImportedRefs_boundary(t *testing.T) {
	...
	exactly21 := time.Now().UTC().AddDate(0, 0, -store.RetentionDays).Format("2006-01-02")
...
func TestPruneImportedRefs_mixed(t *testing.T) {
	...
	refs := map[string]string{
		"ref-old":    now.AddDate(0, 0, -(store.RetentionDays + 5)).Format("2006-01-02"),
...
func TestSaveAndLoadState_roundtrip(t *testing.T) {
	...
	original := &State{
		...
		PendingMap:      map[int64]map[string]string{1: {"REF-ABC": "txn-abc|2024-06-01"}},
		ImportedRefs:    map[int64]map[string]string{1: {"REF-001": "2024-06-01"}},
	}
...
func TestLoadState_initialisesNilMaps(t *testing.T) {
	...
	if s.PendingMap == nil {
		s.PendingMap = make(map[int64]map[string]string)
	}
	if s.ImportedRefs == nil {
		s.ImportedRefs = make(map[int64]map[string]string)
	}
...
func TestState_SetPending_writesThrough(t *testing.T) {
	st := openStateStore(t)
	s := &State{PendingMap: make(map[int64]map[string]string)}
	if err := s.SetPending(1, "k1", "txn-abc", "2024-06-15", st); err != nil {
		t.Fatalf("SetPending: %v", err)
	}
	id, date := splitPendingVal(s.Pending(1)["k1"])
	if id != "txn-abc" {
	...
	dbVal, ok, err := st.GetPendingTxnID(1, "k1")
	if err != nil {
...
func TestState_DeletePending_writesThrough(t *testing.T) {
	st := openStateStore(t)
	s := &State{PendingMap: map[int64]map[string]string{1: {"k1": "txn-abc|2024-06-15"}}}
	_ = st.SetPending(1, "k1", "txn-abc|2024-06-15")
	if err := s.DeletePending(1, "k1", st); err != nil {
		t.Fatalf("DeletePending: %v", err)
	}
	if _, exists := s.Pending(1)["k1"]; exists {
		t.Error("expected removed from memory")
	}
	_, ok, _ := st.GetPendingTxnID(1, "k1")
	if ok {
...
func TestState_AddImportedRef_writesThrough(t *testing.T) {
	st := openStateStore(t)
	s := &State{ImportedRefs: make(map[int64]map[string]string)}
	if err := s.AddImportedRef(1, "REF-1", "2024-06-01", st); err != nil {
		t.Fatalf("AddImportedRef: %v", err)
	}
	if s.Imported(1)["REF-1"] != "2024-06-01" {
		t.Error("expected in memory")
	}
	has, _ := st.HasImportedRef(1, "REF-1")
	if !has {
...
func TestState_PruneImportedRefs_writesThrough(t *testing.T) {
	st := openStateStore(t)
	now := time.Now().UTC()
	oldDate := now.AddDate(0, 0, -(store.RetentionDays + 5)).Format("2006-01-02")
	newDate := now.Format("2006-01-02")
	_ = st.AddImportedRef(1, "old", oldDate)
	_ = st.AddImportedRef(1, "new", newDate)
	s := &State{ImportedRefs: map[int64]map[string]string{1: {"old": oldDate, "new": newDate}}}
	if err := s.PruneImportedRefs(st); err != nil {
		t.Fatalf("PruneImportedRefs: %v", err)
	}
	if _, ok := s.Imported(1)["old"]; ok {
		t.Error("expected old removed from memory")
	}
	if _, ok := s.Imported(1)["new"]; !ok {
...
func TestState_Reload_updatesSession(t *testing.T) {
	st := openStateStore(t)
	_, _ = st.AddBankAccount(store.NewBankAccount{SessionID: "sess-new", AccountUID: "acct-new", BankName: "Bank", BankCountry: "DE", SessionExpiry: "2025-01-01T00:00:00Z"})
	s := &State{}
...
func TestLoadFromStore_withData(t *testing.T) {
	st := openStateStore(t)
	_, _ = st.AddBankAccount(store.NewBankAccount{SessionID: "sess-1", AccountUID: "acct-1", BankName: "Bank", BankCountry: "DE", SessionExpiry: "2025-12-31T00:00:00Z"})
	_ = st.SetLastSyncDate("2024-06-01")
	_ = st.SetPending(1, "REF-1-P", "txn-1|2024-06-01")
	_ = st.AddImportedRef(1, "REF-1", "2024-06-01")
	s, err := LoadFromStore(st)
	if err != nil {
	...
	if s.Pending(1)["REF-1-P"] != "txn-1|2024-06-01" {
		t.Errorf("PendingMap[REF-1-P]: got %q", s.Pending(1)["REF-1-P"])
	}
	if s.Imported(1)["REF-1"] != "2024-06-01" {
		t.Errorf("ImportedRefs[REF-1]: got %q", s.Imported(1)["REF-1"])
	}
}

func TestFindPendingKeyByTxnID_found(t *testing.T) {
	s := &State{PendingMap: map[int64]map[string]string{1: {
		"key-a": "txn-1|2026-07-01",
		"key-b": "txn-2|2026-07-02",
	}}}
	key, ok := s.FindPendingKeyByTxnID(1, "txn-2")
	if !ok {
		t.Fatal("expected to find key for txn-2")
	}
	if key != "key-b" {
		t.Errorf("got %q, want key-b", key)
	}
}

func TestFindPendingKeyByTxnID_notFound(t *testing.T) {
	s := &State{PendingMap: map[int64]map[string]string{1: {"key-a": "txn-1|2026-07-01"}}}
	if _, ok := s.FindPendingKeyByTxnID(1, "txn-999"); ok {
		t.Error("expected no match for unknown txn ID")
	}
}

func TestFindPendingKeyByTxnID_legacyValueWithoutDate(t *testing.T) {
	s := &State{PendingMap: map[int64]map[string]string{1: {"key-legacy": "txn-old"}}}
	key, ok := s.FindPendingKeyByTxnID(1, "txn-old")
	if !ok {
		t.Fatal("expected to find legacy entry by txn ID")
	}
	if key != "key-legacy" {
		t.Errorf("got %q, want key-legacy", key)
	}
}

func TestPrunePendingMap_removesOldKeepsRecent(t *testing.T) {
	st := openStateStore(t)
	s := &State{PendingMap: make(map[int64]map[string]string)}
	oldDate := time.Now().UTC().AddDate(0, 0, -(store.RetentionDays + 5)).Format("2006-01-02")
	recentDate := time.Now().UTC().AddDate(0, 0, -5).Format("2006-01-02")
	if err := s.SetPending(1, "old", "txn-a", oldDate, st); err != nil {
		t.Fatalf("SetPending: %v", err)
	}
	if err := s.SetPending(1, "recent", "txn-b", recentDate, st); err != nil {
		t.Fatalf("SetPending: %v", err)
	}

	if err := s.PrunePendingMap(st); err != nil {
		t.Fatalf("PrunePendingMap: %v", err)
	}

	if _, ok := s.Pending(1)["old"]; ok {
		t.Error("entry older than 21 days must be pruned from memory")
	}
	if _, ok := s.Pending(1)["recent"]; !ok {
		t.Error("recent entry must survive")
	}
	if _, ok, _ := st.GetPendingTxnID(1, "old"); ok {
		t.Error("entry older than 21 days must be pruned from the store")
	}
	if _, ok, _ := st.GetPendingTxnID(1, "recent"); !ok {
		t.Error("recent entry must survive in the store")
	}
}

func TestPrunePendingMap_boundaryKept(t *testing.T) {
	st := openStateStore(t)
	s := &State{PendingMap: make(map[int64]map[string]string)}
	boundary := time.Now().UTC().AddDate(0, 0, -store.RetentionDays).Format("2006-01-02")
	if err := s.SetPending(1, "edge", "txn-e", boundary, st); err != nil {
		t.Fatalf("SetPending: %v", err)
	}
	...
	if _, ok := s.Pending(1)["edge"]; !ok {
		t.Error("entry exactly at the 21-day boundary must be kept, mirroring PruneImportedRefs")
	}
}

func TestPrunePendingMap_legacyValueRemoved(t *testing.T) {
	st := openStateStore(t)
	if err := st.SetPending(1, "legacy", "bare-txn-id"); err != nil {
		t.Fatalf("store SetPending: %v", err)
	}
	s, err := LoadFromStore(st)
	if err != nil {
		t.Fatalf("LoadFromStore: %v", err)
	}
	if _, ok := s.Pending(1)["legacy"]; !ok {
		t.Fatal("setup: legacy entry not loaded")
	}
	...
	if _, ok := s.Pending(1)["legacy"]; ok {
		t.Error("legacy entry without a date must be pruned")
	}
}

func TestPrunePendingMap_empty(t *testing.T) {
	st := openStateStore(t)
	s := &State{PendingMap: make(map[int64]map[string]string)}
	if err := s.PrunePendingMap(st); err != nil {
		t.Fatalf("PrunePendingMap on empty map: %v", err)
	}
}
